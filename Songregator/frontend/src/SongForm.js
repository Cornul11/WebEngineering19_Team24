...
class SearchBar extends Component {
  ...
  fetchUser = e => {
    const url = `http://localhost:8000/songs/${this.state.sendid}/`;
    axios
      .get(url)
      .then(response => {
  ...
  render() {
    ...
    if (this.state.gotData === "true") {
      output = (
        <SongInfo
          id={this.state.fetchUser.song_id}
          title={this.state.fetchUser.song_title}
    ...
    return (
      <div className="container-fluid">
        <div className="form-inline">
          ...
          <button className="btn btn-primary my-2" onClick={this.fetchUser}>
            Search
          </button>
        </div>
        {output}
